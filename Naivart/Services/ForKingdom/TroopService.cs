...
namespace Naivart.Services
{
    public class TroopService
    {
        ...
        {
            var troopStats = await Task.FromResult(UnitOfWork.TroopTypes.Where(x => x.Type == troopType && x.Level == troopTypeLevel).FirstOrDefault());
        ...
        public async Task<ValueTuple<int, string>> UpgradeTroopsAsync(long kingdomId, string username, string type)
        {
            try
            {
                ...
                var upgradedStats = await Task.FromResult(UnitOfWork.TroopTypes.Where(t => t.Type == type && t.Level == troop.TroopType.Level + 1).FirstOrDefault()); //Get stats of particular troop type one level higher than now
